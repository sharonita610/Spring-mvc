...
class ScoreRepositoryImplTest {
    ...
    @Test
    @DisplayName("저장소에서 findAll을 호출하면 그 반환된 리스트에는 성적 정보가 3개 있어야한다.")
    void findAllTest() {
        // given - when - then 패턴
    ...
    @Test
    @DisplayName("나는 저장소에서 findByStuNum을 호출하면 학번이 2인 학생을 조회하면 " +
            "그 학생의 국어 점수가 33점이고 이름은 춘식이어야한다.")
    void findOneTest() {
        // given
        int stuNum = 2;
    ...
    @Test
    @DisplayName("나는 저장소에서 findByStuNum을 호출하면 학번이 99인 학생을 조회하면 " +
            "Null 이 리턴 될 것이다")
    void findOneTest2() {
        // given
        int stuNum = 99;
    ...
    @Test
    @DisplayName("저장소에서 학번이 2인 학생을 삭제한 후에 리스트를 전체 조회해보면 성적의 개수가 2개 일 것이고" +
            "다시 2번 학생을 조회 했을때 null이 반횐되어야한다")
    void deleteTest() {
        // given
        int stuNum = 2;
